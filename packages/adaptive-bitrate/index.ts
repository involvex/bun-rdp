...
export class AdaptiveBitrateController {
  ...
  private readonly STEP_UP = 1.1; // +10 % per evaluation
  ...
  private _evaluate(): void {
    ...
    if (newBitrate !== this.bitrate) {
      console.log(
        `[abr] RTT avg=${avg.toFixed(0)}ms p95=${p95.toFixed(0)}ms → ${(newBitrate / 1000).toFixed(0)}kbps`
      );
  ...
  stats(): { avgRtt: number; p95Rtt: number; bitrate: number } {
    ...
    const avg = this.samples.reduce((s, x) => s + x.rtt, 0) / this.samples.length;
    return {
      avgRtt: Math.round(avg),
      p95Rtt: Math.round(this._percentile(95)),
