...
const FULL_FRAME_THRESHOLD = 0.40;
  ...
  query(): DirtyInfo {
    ...
    try {
      ...
      for (const m of (moveBuf ?? [])) {
        rawMove.push({
          src: { x: m.SourcePoint.x, y: m.SourcePoint.y, w: m.DestinationRect.right - m.DestinationRect.left, h: m.DestinationRect.bottom - m.DestinationRect.top },
          dst: { x: m.DestinationRect.left, y: m.DestinationRect.top, w: m.DestinationRect.right - m.DestinationRect.left, h: m.DestinationRect.bottom - m.DestinationRect.top },
        });
        // Treat move destination as dirty too
        rawDirty.push({ x: m.DestinationRect.left, y: m.DestinationRect.top, w: m.DestinationRect.right - m.DestinationRect.left, h: m.DestinationRect.bottom - m.DestinationRect.top });
      }
    } catch {
