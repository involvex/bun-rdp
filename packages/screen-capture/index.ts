import { dxgi, d3d11, User32, Gdi32 } from 'bun-win32';
...
class DxgiCapture {
  ...
  init(monitorIndex = 0): boolean {
    try {
      ...
      const result = d3d11.D3D11CreateDevice(adapter, d3d11.D3D_DRIVER_TYPE_UNKNOWN,
        null, 0, [d3d11.D3D_FEATURE_LEVEL_11_0], d3d11.SDK_VERSION);
  ...
  capture(timeoutMs = 16): CaptureFrame | null {
    ...
    try {
      ...
      if (!fi || fi.LastPresentTime === 0n) { this.duplication.ReleaseFrame(); return null; }
      ...
      const staging = this.device.CreateTexture2D({
        Width: this.width, Height: this.height, MipLevels: 1, ArraySize: 1,
        Format: dxgi.DXGI_FORMAT_B8G8R8A8_UNORM, SampleDesc: { Count: 1, Quality: 0 },
        Usage: d3d11.D3D11_USAGE_STAGING, BindFlags: 0,
        CPUAccessFlags: d3d11.D3D11_CPU_ACCESS_READ, MiscFlags: 0,
      }, null);
      ...
      return { data: out, width: this.width, height: this.height, timestamp: Date.now() };
    } catch { return null; }
  }

  dispose() { this.duplication = null; this.device = null; }
}
  ...
  capture(): CaptureFrame | null {
    ...
    try {
      ...
      if (ci.flags === User32.CURSOR_SHOWING)
        User32.DrawIconEx(this.hdcMem, ci.ptScreenPos.x, ci.ptScreenPos.y, ci.hCursor, 0, 0, 0, null, User32.DI_NORMAL);
      const bmi = new Gdi32.BITMAPINFO({ biWidth: this.width, biHeight: -this.height, biBitCount: 32, biCompression: Gdi32.BI_RGB });
      ...
      return { data: buf, width: this.width, height: this.height, timestamp: Date.now() };
    } catch { return null; }
  }
...
export class ScreenCapture {
  ...
  dispose() { this.backend?.dispose(); this.backend = null; }
}
