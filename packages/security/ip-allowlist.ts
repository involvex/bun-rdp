...
function ipv6ToBigInt(ip: string): bigint {
  ...
  return expanded
    .split(':')
    .reduce((acc, grp) => (acc << 16n) | BigInt(Number.parseInt(grp || '0', 16)), 0n);
}
...
export class IPAllowlist {
    ...
    this.rules = entries
      .map((e) => {
        try {
          ...
          console.warn(`[allowlist] Invalid rule "${e}": ${err}`);
          return null;
        }
      })
      .filter(Boolean) as ParsedRule[];
    console.log(
      `[allowlist] ${this.rules.length} rule(s): ${this.rules.map((r) => r.raw).join(', ')}`
    );
  }
  ...
  isAllowed(ip: string): boolean {
    ...
    try {
      parsed = ipToInt(ip.replace(/^\[/, '').replace(/\]$/, ''));
    } catch {
      return false;
    }
  ...
  addRule(cidr: string): void {
    try {
      ...
      console.warn(`[allowlist] Invalid: ${cidr} — ${e}`);
    }
  }
  ...
  get ruleCount() {
    return this.allowAll ? Number.POSITIVE_INFINITY : this.rules.length;
  }
}
